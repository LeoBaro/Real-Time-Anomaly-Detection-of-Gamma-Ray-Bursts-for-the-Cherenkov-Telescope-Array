...
if [ $# -lt 1 ]; then
    echo "Usage: $0 <do_simulation> <do_photometry> <do_plots>\n\n Example: $0 1 1 1"
    exit 1
fi
...
if [ "$1" = "1" ]; then
    ...
    output_dir_sim="$base_output_dir/fits_data"
    simGRBcatalogWithRandomization -f $this_dir/config.yml --output-dir $output_dir_sim --print no --mp-threads 50 2>&1 > $this_dir/simulation.log
fi
