...
class Photometry:
    ...
    def getLogaritmicWindows(self, window_start, window_stop, num):

        _windows = np.logspace(window_start, window_stop, num=num)
    ...
    def integrate(self, photometry_params, sim_params, integration, windowsInterval="linear"):
        ...
        elif windowsInterval == "logarithmic":
            time_windows = self.getLogaritmicWindows(sim_params["t_window_start"], sim_params["t_window_stop"], )
            energy_windows = self.getLogaritmicWindows(sim_params["e_window_start"], sim_params["e_window_stop"], )
        else:
            raise ValueError(f"windowsInterval={windowsInterval} is not supported!")
